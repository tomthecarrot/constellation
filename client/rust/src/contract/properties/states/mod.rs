...
#[cfg(feature = "c_api")]
#[rsharp::substitute("tp_client::contract::properties::states")]
pub mod c_api {
        ...
        ($path:literal, $t:ty $(,)?) => {
            paste::paste! {
                ...
                mod [<_State _ $t:camel>] {
                    ...
                    #[remangle($path)]
                    #[ffi_export]
                    pub fn [<State _ $t:camel __value_set>]<'a>(state: &'a Monomorphized, new_value: Box<$t>) {
                        state.inner.value = new_value;
                    }
                }
            }
        };
